...
class WxPayDataBase {
	...
	public function MakeSign() {
		...
		//签名步骤二：在string后加入KEY
		$string = $string . "&key=" . $weixin_settings['key'];
		//签名步骤三：MD5加密
		$string = md5($string);
		//签名步骤四：所有字符转为大写
 ...
 *
 */
class WxPayNativeApiPay extends WxPayDataBase {
	/**
	 * 设置微信分配的公众账号ID
	 * @param string $value
	 **/
	public function SetCodeurl($value) {
		$this->values['codeurl'] = $value;
	}
	/**
	 * 获取微信分配的公众账号ID的值
	 * @return 值
	...
	public function IsCodeurlSet() {
		return array_key_exists('codeurl', $this->values);
	}
